...
class PhotoCanvas extends JComponent {
  ...
  override def getPreferredSize = {
    new Dimension(image.ancho, image.alto)
  }
  ...
  def applyFilter(filterName: String, numTasks: Int, radius: Int) {
    val dst = new Img(image.ancho, image.alto)
    filterName match {
      case "desenfoque-horizontal" =>
        DesenfoqueHorizontal.desenfoquePar(image, dst, numTasks, radius)
      case "desenfoque-vertical" =>
        DesenfoqueVertical.desenfoquePar(image, dst, numTasks, radius)
      case "" =>
    }
    image = dst
    repaint()
  ...
  override def paintComponent(gcan: Graphics) = {
    ...
    val width = image.ancho
    val height = image.alto
